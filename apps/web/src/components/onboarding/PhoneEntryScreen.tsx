...
const PhoneEntryScreen = ({
  ...
  return (
    <OnboardingShell
      ...
      helpActionLabel="Vi sao can phone"
      onHelpAction={onWhyPhoneOpen}
      footer={
        <p className="px-2 text-center text-xs leading-5 text-white/45">
          So dien thoai se duoc dung de tao session va giam gian lan onboarding.
        </p>
      }
    >
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div className="rounded-2xl border border-emerald-400/12 bg-emerald-400/8 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-white">Nhan OTP tu backend dev</p>
              <p className="text-xs text-white/56">Khong can SMS vendor trong local demo.</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-3 text-sm text-white/68">
          ...
          Recovery va giam duplicate se bat dau tu so dien thoai nay.
        </div>
        ...
        >
          {isSubmitting ? "Dang gui OTP..." : view.primaryActionLabel}
        </Button>
      </form>
    </OnboardingShell>
  );
