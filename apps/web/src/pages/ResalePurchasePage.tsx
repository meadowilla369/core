...
const ResalePurchasePage = () => {
  ...
  const isListingActive = listing.listingStatus === "active";

  const prepareBuyMutation = useMutation({
    mutationFn: async () => {
      if (!ticketId) throw new Error("Missing listing id");
      if (!onChainListingId) {
        throw new Error(
          "Listing chưa có on-chain ID. Contract-sync có thể chưa index. Thử lại sau vài giây."
        );
      }
      ...
      const txDraft = buildMarketplaceBuyTx({
        ...
        listingId: BigInt(onChainListingId),
      ...
      return { backend: response.data, txDraft };
    },
    onSuccess: (result) => {
      setPrepared(result);
      setBroadcasted(null);
    ...
    onError: (error) => {
      toast({
        ...
        description: error instanceof Error ? error.message : "Không thể chuẩn bị lệnh mua",
        variant: "destructive"
      });
    }
  });
  ...
  const broadcastBuyMutation = useMutation({
    mutationFn: async () => {
      if (!ticketId || !prepared) throw new Error("Buy flow has not been prepared");
    ...
    onError: (error) => {
      toast({
        ...
        description: error instanceof Error ? error.message : "Không thể ký và broadcast",
        variant: "destructive"
      });
    }
  });
  ...
  const copyJson = async (value: unknown, label: string) => {
    ...
    toast({ title: "Đã copy", description: `${label} đã được copy vào clipboard` });
  };
  ...
  return (
    <MobileLayout>
      ...
      {/* On-chain listing ID — read-only, từ contract-sync */}
      <section className="px-4 pb-4">
        <h3 className="font-mono text-[10px] tracking-widest text-foreground/50 mb-3">
          [ ON-CHAIN LISTING ]
        </h3>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="font-mono text-[10px] text-foreground/40 mt-1">
                  Được đọc tự động từ contract-sync. Dùng để build `buyWithSignature()`.
                </p>
              </div>
            </div>
          ) : (
        ...
        <section className="px-4 pb-6 space-y-4">
          ...
          {broadcasted && (
            <div className="border border-foreground/20 p-4 space-y-4">
              ...
              <div className="p-3 border border-green-600/30 bg-green-600/5">
                <p className="font-mono text-[10px]">
                  Listing ID: <span className="break-all">{broadcasted.backend.listingId}</span>
                </p>
                <p className="font-mono text-[10px] mt-1">
                  Buy hash order: {broadcasted.backend.buyHashOrderId}
                </p>
                <p className="font-mono text-[10px] mt-1">
                  TX hash: <span className="break-all">{broadcasted.backend.txHash ?? "—"}</span>
                </p>
              </div>
